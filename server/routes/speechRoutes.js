...
router.post('/analyze-speech', async (req, res) => {
  try {
    ...
    // Shared scoring criteria for both retake FULL ANALYSIS and non-retake modes
    const fullAnalysisScoringCriteria = `
    ...
    const retakePrompt = `
      ...
      **DETECTION CRITERIA:**
      - **DELIVERY MODE** (reading/practicing the reference): User's speech uses ≥85% of the same words/phrases from the reference
        → Same vocabulary, sentence structures, and order
        → May have minor pronunciation differences, filler words, or small omissions
        → They are READING or RECITING the improved version
        → Use detected_framework = "PRACTICE_DELIVERY"

      - **FULL ANALYSIS MODE** (new attempt): User's speech uses <85% word overlap with reference
        → Different vocabulary, examples, sentence structures, or ideas
        → They gave a NEW answer (not just reading the reference)
        → Analyze it like a first-time response
        → Use detected_framework = actual framework (STAR/PREP/etc)

      ---

      ## IF DELIVERY MODE (practicing the reference):
      **GOAL:** Evaluate ONLY how well they delivered the speech (pronunciation, fluency, confidence). Do NOT analyze content structure or logic.

      - detected_framework: "PRACTICE_DELIVERY"
      - Score from 0-100 (NOT 0-10!) based on DELIVERY quality ONLY:
        - 85-100: Excellent delivery - smooth, confident, natural pacing, clear pronunciation
        - 70-84: Good delivery - mostly fluent with minor hesitations or pronunciation slips
        - 55-69: Developing - noticeable pauses, some words mispronounced or missed
        - Below 55: Needs more practice - frequent pauses, unclear pronunciation, low confidence
      - improved_transcription: Keep the SAME as the reference (they're already practicing the ideal version)
      - improved_structure: Keep the SAME as the reference structure (no re-analysis needed)
      - Feedback focuses ONLY on: pacing, pronunciation, intonation, confidence, fluency
      - Do NOT critique content, logic, or structure (that was already improved)
      - improvements array: empty [] (no language polish needed - they're reading the improved version)
      - IMPORTANT: Do NOT mention "PRACTICE_DELIVERY" or framework names in feedback text - just give natural delivery coaching
      ${level === 'Easy' ? `
      ...
      **GOAL:** Analyze this as a BRAND NEW response. Use THE EXACT SAME analysis process as first-time (non-retake) mode.
