...
const PyramidNode = memo(({ data }: { data: any }) => {
    const getBadge = () => { if (data.type === 'story') return { label: '📖 Story', bg: 'bg-purple-100 text-purple-700' }; if (data.type === 'fact') return { label: '📊 Fact', bg: 'bg-blue-100 text-blue-700' }; if (data.type === 'opinion') return { label: '💭 Opinion', bg: 'bg-orange-100 text-orange-700' }; return null; };
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-6 space-y-8 md:space-y-12 relative pb-32">
      ...
      <div className="text-center space-y-6">
        <div className="space-y-2">
            ...
            {detected_framework && <div className="inline-block mt-2 px-3 py-1 bg-indigo-50 text-indigo-700 text-[10px] md:text-xs font-bold uppercase tracking-wider rounded-full border border-indigo-100">Detected: {detected_framework.replace(/_/g, ' ')}</div>}
        </div>
            ...
            {selectedNodeData && (
                <div className="absolute bottom-4 left-4 right-4 md:left-auto md:right-4 md:w-80 bg-white rounded-xl shadow-xl border border-stone-200 p-4 animate-in slide-in-from-bottom-4 fade-in duration-300 z-20">
                    <div className="flex justify-between items-start mb-2">
                        <div className="flex items-center gap-2"><div className={`p-1 rounded-full ${viewMode === 'ai' ? 'bg-sky-100' : 'bg-amber-100'}`}><svg className={`w-3 h-3 ${viewMode === 'ai' ? 'text-sky-600' : 'text-amber-600'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div><span className="font-bold text-stone-800 text-xs md:text-sm">{viewMode === 'ai' ? 'Elaboration' : 'Critique'}</span></div>
